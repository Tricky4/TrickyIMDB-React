        ...
        <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
          ...
          <p className="my-7 flex flex-wrap gap-2">
            { movie.genres?.map((genre) => (
              <span className="mr-2 border border-gray-200 rounded fark:border-gray-600 p-2" key={genre.id}>{genre.name}</span>
            )) }
          </p>
          ...
          <p className="my-4">
            ...
            <a className="text-gray-900 underline hover:no-underline dark:text-white" href={`https://www.imdb.com/title/${movie.imdb_id}`} target="_blank" rel="noreferrer">{movie.imdb_id}</a>
          </p>
